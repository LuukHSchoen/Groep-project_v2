...
public class PresentieAanpassenController implements Handler {
	...
	private void presentiesaanpassen(Conversation conversation) {
		...
		for (String klascode : deklassen) {
			JsonObjectBuilder lJsonObjectBuilderklascodes = Json.createObjectBuilder(); // maak
																						// het
																						// JsonObject
																						// voor
																						// een
																						// student
			lJsonObjectBuilderklascodes.add("klascode", klascode);

			lJsonArrayBuilder.add(lJsonObjectBuilderklascodes); // voeg het
																// JsonObject
																// aan het array
																// toe
		}

		String lJsonOutklascodes = lJsonArrayBuilder.build().toString(); // maak
																			// er
																			// een
																			// string
																			// van
	...
	private void sessiesaanpassen(Conversation conversation) {
		...
		for (Sessie deSessie : desessies) {
			...
			JsonObjectBuilder lJsonObjectBuilderString = Json.createObjectBuilder(); // maak
																						// het
																						// JsonObject
																						// voor
																						// een
																						// student
			lJsonObjectBuilderString.add("destring", samen); // vul het
																// JsonObject

			lJsonArrayBuilder.add(lJsonObjectBuilderString);

		}
		String lJsonOutDesessies = lJsonArrayBuilder.build().toString(); // maak
																			// er
																			// een
																			// string
																			// van
