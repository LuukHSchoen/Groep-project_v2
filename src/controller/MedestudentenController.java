...
public class MedestudentenController implements Handler {
	 ...
	 * juiste methode het werk doen. Je kunt voor elke nieuwe URI een nieuwe
	 * methode schrijven.
	 * 
	 * @param infoSys
	 *            - het toegangspunt tot het domeinmodel
	 */
	...
	private void ophalen(Conversation conversation) {
		...
		Klas lKlas = informatieSysteem.getKlasVanStudent(lStudentZelf); // klas
																		// van
																		// de
																		// student
																		// opzoeken

		ArrayList<Student> lStudentenVanKlas = lKlas.getStudenten(); // medestudenten
																		// opzoeken

		JsonArrayBuilder lJsonArrayBuilder = Json.createArrayBuilder(); // Uiteindelijk
																		// gaat
																		// er
																		// een
																		// array...

		for (Student lMedeStudent : lStudentenVanKlas) { // met daarin voor elke
															// medestudent een
															// JSON-object...
			if (lMedeStudent == lStudentZelf) // behalve de student zelf...
				...
				JsonObjectBuilder lJsonObjectBuilderVoorStudent = Json.createObjectBuilder(); // maak
																								// het
																								// JsonObject
																								// voor
																								// een
				...
				lJsonObjectBuilderVoorStudent.add("id", lMedeStudent.getStudentNummer()) // vul
																							// het
																							// JsonObject
						.add("firstName", lMedeStudent.getVoornaam()).add("lastName", lLastName)
						.add("sameGroup", lZelfdeGroep);

				lJsonArrayBuilder.add(lJsonObjectBuilderVoorStudent); // voeg
																		// het
																		// JsonObject
																		// aan
																		// het
		...
		String lJsonOutStr = lJsonArrayBuilder.build().toString(); // maak er
																	// een
																	// string
																	// van
		conversation.sendJSONMessage(lJsonOutStr); // string gaat terug naar de
													// Polymer-GUI!
	}

	/**
	 * Deze methode haalt eerst de opgestuurde JSON-data op. Op basis van deze
	 * gegevens het domeinmodel gewijzigd. Een eventuele errorcode wordt
	 * tenslotte weer (als JSON) teruggestuurd naar de Polymer-GUI!
	 * 
	 * @param conversation
	 *            - alle informatie over het request
	 */
	private void opslaan(Conversation conversation) {
		...
		// Het lJsonObjectIn bevat niet enkel strings maar ook een heel (Json)
