...
public class PresentieInzienController implements Handler {
	...
	private void presentiesaanpassen(Conversation conversation) {
		...
		for (Klas deklas : deklassen) {
			...
			JsonObjectBuilder lJsonObjectBuilderspresentie = Json.createObjectBuilder(); // maak
																							// het
																							// JsonObject
																							// voor
																							// een
																							// student
			lJsonObjectBuilderspresentie.add("present", present).add("absent", absent).add("klascode",
					deklas.getKlasCode());
